...
export class SimController {
  ...
  constructor(gs, opts = {}) {
    ...
    /**
     * WAS THERE AN OUT, THIS FRAME?
     *
     * The whole point of a utility controller is that it scores every action
     * available before picking one — so "could ANY input have avoided this?"
     * is already computed and thrown away. Keeping it turns unavoidable damage
     * from a judgement call into a byproduct: if no candidate was clean, a hit
     * on this frame was undodgeable, and that is the number that separates a
     * boss being unfair from a boss being hard.
     *
     * Read by the runner AFTER gs.step(), which is the correct order — the
     * decision is made before the frame resolves.
     */
    this.hadEscape = true;
    this.noEscapeFrames = 0;

    /**
     * INPUTS, counted as a human would press them: transitions, not frames.
     * Holding right for a second is one input, not sixty.
     */
    this.inputs = 0;
    this.lastDir = 0;
  ...
  move(threats) {
    ...
    for (const dir of [-1, 0, 1]) {
      for (const jump of canJump ? [false, true] : [false]) {
        const { s, danger } = this.score(threats, dir, jump, false);
        if (danger === 0) anyClean = true;
        if (!best || s > best.s) best = { s, dir, jump, slide: false };
      }
    }
    if (canSlide) {
      const { s, danger } = this.score(threats, p.facing, false, true);
      if (danger === 0) anyClean = true;
    ...
    // Recorded, not acted on. See the note in the constructor.
    this.hadEscape = anyClean;
    if (!anyClean) this.noEscapeFrames++;

    // COUNTED AS PRESSES. A change of direction is one input; holding it is
    // free. Jump and slide are discrete by nature.
    if (best.dir !== this.lastDir) { this.inputs++; this.lastDir = best.dir; }
    if (best.jump) this.inputs++;
    if (best.slide) this.inputs++;
  ...
  score(threats, dir, jump, slide) {
    ...
    // BOTH HALVES, SEPARATELY. The blended number picks the action; the raw
    // danger says whether that action was actually clean, which is what makes
    // "no input could have avoided this" answerable.
    return { s: good - danger * DANGER_W, danger };
  }
  ...
  shoot() {
    ...
    if (!gs.intent.fireHeld) { gs.beginFire(); this.inputs++; }
