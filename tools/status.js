...
const byName = Object.fromEntries(tracker.bosses.map((b) => [b.name.toUpperCase(), b]));
const furnished = new Set(FURNISHED());

const DESIGN_FIELDS = [
  'arenaTheme', 'hazardsLayer1', 'hazardsLayer2', 'hazardsLayer3',
  'attackLayer1', 'attackLayer2', 'attackLayer3',
  'weaponDescription', 'lv1', 'lv3', 'lv6', 'lv10',
];
...
const rows = BOSSES.map((b) => {
  const t = byName[b.name] || {};
  const written = DESIGN_FIELDS.filter((k) => t[k]?.text).length;
  const drafts = DESIGN_FIELDS.filter((k) => t[k]?.text?.startsWith('[DRAFT]')).length;
  ...
  return {
    b, t, written, drafts, atk, haz, wpn,
...
console.log(`slices complete: ${done}/17   started: ${started}/17   `
  + `tracker fields still [DRAFT]: ${drafts}`);
...
if (drafts) {
  console.log(`\n${drafts} tracker fields are generated drafts awaiting review.`);
  console.log(`A slice is not DONE while any of its design is still a draft.`);
}
console.log('');
