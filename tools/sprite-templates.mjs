...
// The rooms themselves, so furniture sizes are measured rather than described.
import { makeArena } from '../src/systems/arena.js';
 ...
 */
/**
 * EVERY PIECE OF ARENA FURNITURE, MEASURED BY BUILDING THE ROOM.
 *
 * `makeArena` is run for each boss at layer 3 — the layer that builds the most,
 * since Blaze Man's lift and Volt Man's conductors only exist above layer 1 —
...
for (const b of bossRows) {
  const room = makeArena(BOSS_BY_ID[b.id], 3, 400, 180);
  const pieces = [];
  for (const kind of FURNITURE_KINDS) {
    const v = room[kind];
    if (!v) continue;
    const one = Array.isArray(v) ? v[0] : v;
    // Only a piece with real dimensions can be drawn at a fixed grid. A
    // full-width feature (Strike Man's rails, Tempest Man's drain) is the room
    // rather than an object in it, so it gets no canvas here.
    if (!one || typeof one !== 'object' || !one.w || !one.h) continue;
    /**
     * THE CANVAS COVERS WHAT IS DRAWN, NOT WHAT IS LOGGED.
     *
     * Two pieces draw past their own w/h and a grid taken from the number
     * would clip them: a turret hangs a 3px muzzle BELOW its 10x7 body, and
     * ground cover at full growth stands 12px out of a 6px strip. Both are
     * the sprite-box/collision-box split showing up in furniture, so the
     * canvas takes the drawn extent and the piece keeps its own dimensions.
     */
    const grow = { turrets: { h: 3 }, cover: { h: 6 } }[kind];
    pieces.push({ kind, w: one.w + (grow?.w || 0), h: one.h + (grow?.h || 0) });
  }
  pieces.slice(SLOTS_PER_ARENA).forEach((p) => overflow.push(`${b.id}.${p.kind}`));
  for (let i = 0; i < SLOTS_PER_ARENA; i++) {
    const piece = pieces[i];
    furnitureRows.push({
      id: `furniture-${b.id}-${i + 1}`,
      key: `furniture:${b.id}:${i + 1}`,
      boss: b.id,
      label: `${b.name} ${piece ? piece.kind.toUpperCase() : `SPARE ${i + 1}`}`,
      grid: piece ? { w: piece.w, h: piece.h } : { w: SHOT_GRID, h: SHOT_GRID },
      spare: !piece,
    });
  }
}
...
const targets = {
   ...
   * is 44x5 because that is what Blaze Man's room builds, and a speaker is
   * 22x54 because that is what Volt Man's does. A slot with no piece behind it
   * yet is a SPARE at the shot grid — somewhere to draw a new piece before the
   * code that places it exists.
   *
   * NO COLLISION BOX, for the same reason a shot has none: furniture collision
   * is whatever its own hazard loop tests, not a per-actor hurtbox.
   */
  furniture: Object.fromEntries(furnitureRows.map((f) => [f.id, {
    label: f.label, cls: 'furniture', key: f.key, boss: f.boss,
    grid: f.grid, box: null, spare: f.spare,
    frames: ['idle0'],
  }])),
  /**
