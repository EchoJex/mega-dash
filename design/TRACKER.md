...
- **hazard L2** `[wip]` Per sweep: after final panel has finished discharging, the arena brightness flickers dark normal dark, staying dark. then after the last of the electrified panels have finished being electrified the two power lines sparkle for 1s then dischargeare inert between arcs and can be stood under safely.
