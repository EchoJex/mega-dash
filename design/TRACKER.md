...
- **hazard L3** `[wip]` Rain changes direction with limited die down between direction changes, with a random duration of at least 3s. Lightning. Both portholes run at full pressure; floor water rises to mid-shin and the inward current roughly doubles, so holding station away from the drain takes constant input.
