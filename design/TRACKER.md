...
- **weapon** `[wip]` Defensive; a small gray drone hovers just above and in front of the player's shoulder. It continuously auto aims at the nearest enemy and auto fires a neutral bullet with clip cooldown time is equal to the current levels max ammo shots per second, fire rate and with a low base damage.
- **weapon Lv1** `[wip]` Single shot, mild damage, 2 shots per second, 10 ammo clip. Clip cooldown
