...
- **weapon** `[wip]` Offensive; Large rock shaped hammer visible when active. Slow, delayed baseball-swing on tap for high damage and high knockback, long press hold hammer overhead and on release swings downwardfor overhead Ground-pound that creates shockwaves and stuns nearby enemies on long press (0.4s). Per-level scaling: shockwave size + stun duration.
