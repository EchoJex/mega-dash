...
- **hazard L3** `[wip]` Rain changes direction with limited die down between direction changesith no settled beat. Both portholes run at full pressure; floor water rises to mid-shin and the inward current roughly doubles, so holding station away from the drain takes constant input.
