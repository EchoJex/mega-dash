...
- **weapon Lv10** `[wip]` Chain damage hits up to 3 nearby enemies near the first enemy contacted; which then continue to chain up to 2 additional nearby enemies, which turn continue to chain to up to 1 additional enemy.
- **silhouette** `[todo]` Deferred — silhouette follows from attack + arena design, not before it.
