...
- **weapon** `[wip]` Offensive; Large rock shaped hammer. Slow swing on tap high damage high knockback, overhead Ground-pound that creates shockwaves and stuns nearby enemies on long press (0.4s). Per-level scaling: shockwave size + stun duration.
- **weapon Lv1** `[wip]` Ground-pound on landing from a jump; small shockwave both directions, brief Stun on contact.
