...
| build | status | bug |
|---|---|---|
| 1036 (main) | `[wip]` | When falling into a pit, the player will pop back onto safe ground so long as they are pushing toward the edge. By default I want there to be nearly zero tolerance for pit collision detection, similar to the almost-but-not-quite tolerance for incoming bullet hurt box detection. However! I love this current functionality as a meta progression feature called Cliff edge Mastery, where once unlocked the cloud collision become even more generous to the player, with |
