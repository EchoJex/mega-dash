...
- **arena furniture** `[wip]` Four phasing platforms that phase in at a new location each time they phase IN, 2 at a time, 3 seconds apart, each set lasting 5 seconds. eight floor panels. Two large industrial speakerstwo long industrial power lines at 30% and 70% arena width. The cables are drawn at every layer and but the conductors and the sparkle are only drawn at layer 2 and layer 3.
