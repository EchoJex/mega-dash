...
- **palette notes** `[wip]` Basically a Blastoise with two step hydro cannons
- **arena** `[wip]` Background theme High seas, tempest
- **attribute (boss)** `[ready]` None
- **attribute (weapon)** `[wip]` Wet: the surface or enemy has been soaked in water, reducing contact friction if terrain and reducing enemy movement speed of enemy. and visually indicated by an initially low transparency blue hue that fades until the attribute has expired. Lasts 10 seconds.
