...
- **hazard L1** `[wip]` Heavy rain pouring straight down top-to-bottom, applies a continuous directional force vector / velocity bias that pushes player in the direction of the rain.
  ...
  Implement appropriate 2D physics: the flowing water applies a moderate continuous directional force / velocity bias that drags Mega Man toward the central drain. a damaging spike ball sits on top the grate that the player should avoid. Combine with the diagonal storm rain push for layered pressing pressure the player must resist."
