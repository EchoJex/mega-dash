...
- **attribute (boss)** `[ready]` None
- **attribute (weapon)** `[wip]` Wet: the surface or enemy has been soaked in water, reducing contact friction if terrain and reducingand visually indicated by an initially low transparency blue hue that fades until the attribute has expired. Lasts 10 seconds.
