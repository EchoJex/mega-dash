...
- **post-boss mode** `[wip]` Opens by itself after the boss detect animationwhen a boss falls. Hard pause, HUD hidden, every unlocked benched weapon at full strength, and the only place the loadout can be rearranged. Two taps in either order — a weapon then a module, or a module then a weapon.
