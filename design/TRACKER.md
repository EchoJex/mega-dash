...
- **weapon** `[wip]` Defensive; a small gray drone hovers just above and in front of the player's shoulder. It continuously auto aims at the nearest enemy and auto fires a neutral bullet with clip cooldown time equal to one and a half times the current levels clip size divided by the current levels shot per second (clip_cooldown=1.5(clip_size/fire_rate)). fire rate and with a low base damage.
